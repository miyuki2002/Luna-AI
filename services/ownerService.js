...
class OwnerService {
    ...
    async loadOwnerInfo() {
        if (!this.ownerId || !this.client) {
            logger.warn('OWNER', 'OWNER_ID không được thiết lập hoặc client chưa sẵn sàng');
            return;
        }
        ...
        try {
            ...
            logger.info('OWNER', `Đã tải thông tin owner: ${this.ownerInfo.username} (${this.ownerInfo.id})`);
        } catch (error) {
    ...
    isOwnerMentioned(content, message = null) {
        ...
        if (message && message.mentions && message.mentions.users.has(this.ownerId)) {
            return true;
        }
        ...
        const ownerKeywords = [
            'owner', 'chủ sở hữu', 'người sáng lập', 'creator', 'developer', 'dev',
            'người tạo', 'người phát triển', 'admin chính', 'boss'
        ];
    ...
    async getOwnerMentionResponse(context = '') {
        ...
        try {
            ...
            logger.error('OWNER', 'Lỗi khi tạo phản hồi động cho owner mention:', error);
