    ...
    if (customId === 'resetdb_confirm') {
      ...
      if (success) {
        await interaction.editReply({
          content: '✅ **Đã reset database thành công!**\n\n' +
        ...
        await interaction.editReply({
          content: '❌ **Lỗi khi reset database!**\n\n' +
      ...
      logger.info('RESETUSER', `Owner ${user.tag} đã xác nhận reset user database`);
      ...
      try {
        ...
        await interaction.editReply({
          content: '✅ **Đã reset user database thành công!**\n\n' +
        ...
        await interaction.editReply({
          content: '❌ **Lỗi khi reset user database!**\n\n' +
                   '> Có lỗi xảy ra trong quá trình reset\n' +
                   '> Vui lòng kiểm tra logs để biết thêm chi tiết\n' +
                   '> Liên hệ admin nếu vấn đề tiếp tục',
          components: []
      ...
      await interaction.update({
        content: '❌ **Đã hủy reset user database!**\n\n' +
                 '> Không có thay đổi nào được thực hiện\n' +
