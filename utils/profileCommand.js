...
async function handleProfileCommand(interaction) {
  ...
  try {
    ...
    if (!serverData) {
      return interaction.editReply(
        `❌ **${member.user.username}** chưa bắt đầu nhận XP trong server này!`
      );
    }
    ...
    const allProfiles = await profileCollection
      .find({
      ...
      allGlobalProfiles
        .map((profile) => {
          ...
          if (profile.data.xp && Array.isArray(profile.data.xp)) {
            totalXP = profile.data.xp.reduce(
              (sum, xpData) => sum + xpData.xp,
              0
            );
          }
